...
class ProjectContractorTask(models.Model):
    ...
    contractor_id = fields.Many2one('project.contractor', string="Contractor", ondelete="cascade", track_visibility="always")
    task_id = fields.Many2one('project.task', string="Tasks/activities", required=True, domain="[('project_id', '=', project_id), ('boq_id', 'not in', [False])]", ondelete="cascade", track_visibility="always")
    phase_id = fields.Many2one('project.phase', string="Phase/Division", store=True, related="task_id.phase_id")
...
class ProjectContractor(models.Model):
    ...
    def _compute_amount(self):
        for i in self:
            ...
            total_billed = sum(rec.total_amount for rec in self.env['project.contractor.bill']. search([('project_contract_id', '=', i.id), ('state', '!=', 'canceled')]))
            i.total_billed = total_billed
            i.total_billable = total_amount - total_billed

    name = fields.Char(string="Reference", default='/')
    currency_id = fields.Many2one('res.currency', related='project_id.company_id.currency_id', string="Company Currency")
    project_id = fields.Many2one('project.project', string="Project", required=True, domain="[('project_type', '=', 'project')]", readonly=True, states={'draft': [('readonly', False)]})
    partner_id = fields.Many2one('res.partner', string="Contractor", domain="[('supplier', '=', True)]", required=True, readonly=True, states={'draft': [('readonly', False)]})
    contract_doc = fields.Binary(string="Memorandum", track_visibility="always", readonly=True, states={'draft': [('readonly', False)]})
    contractor_task_ids = fields.One2many('project.contractor.task', 'contractor_id', string="Tasks", readonly=True, states={'draft': [('readonly', False)]})
    bill_complete_only = fields.Boolean(string="100% completion should be billed only", readonly=True, states={'draft': [('readonly', False)]})
    date = fields.Date(string="Date", default=date.today(), required=True, readonly=True, states={'draft': [('readonly', False)]})
...
class ProjectContractorBillTask(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', related='task_id.project_id.company_id.currency_id', string="Company Currency")
...
class ProjectContractorBill(models.Model):
    ...
    name = fields.Char(string="Reference", default='/')
    project_contract_id = fields.Many2one('project.contractor', string="Contract", required=True, domain="[('partner_id', '=', partner_id)]", readonly=True, states={'draft': [('readonly', False)]})
    project_id = fields.Many2one('project.project', string="Project", store=True, related="project_contract_id.project_id")
    currency_id = fields.Many2one('res.currency', related='project_id.company_id.currency_id', string="Company Currency")
    partner_id = fields.Many2one('res.partner', string="Contractor", domain="[('supplier', '=', True)]", required=True, readonly=True, states={'draft': [('readonly', False)]})
    bill_date = fields.Date(string="Billing Date", required=True, default=date.today(), states={'draft': [('readonly', False)]})
    note = fields.Text(string="Notes")
    line_ids = fields.One2many('project.contractor.bill.task', 'bill_id', string="Activities")
    billing_history = fields.Many2many('project.contractor.bill', 'history_contractor_bill_rel', 'bill_id', 'history_id', string="History")
    total_amount = fields.Monetary(string="Total Amount", compute="_compute_amount")
    ...
    def _prepare_invoice_line(self, type):
        ...
        res = []
        if type == 'in_invoice':
            product = self.env.ref('construction_contractor_billing.product_project_contractor_billing', raise_if_not_found=False)
            if not product:
                return False
        ...
        for i in self.line_ids:
            ...
            name = '%s:\n %spercent accomplishment for billing cycle %s'%(i.task_id.name, i.billable_accomplishment, self.bill_date.strftime(DF))
            res += [(0, 0, {
