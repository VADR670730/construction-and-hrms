        ...
        for i in self:
            ...
            if project.project_type == "project" and project.parent_id:
                ...
                for accom in project.parent_id.projection_accomplishment_ids:
                    porfolio_dates.append(accom.date)
                if i.survey_frequent == 'week':
                    if porfolio_dates:
                        if min(porfolio_dates) > i.start_date or max(porfolio_dates) < i.start_date + relativedelta(days= (7*i.number_of_frequent)):
                            raise ValidationError(_("Project timeline should be within Project's Porfolio timeline %s to %s"%(min(porfolio_dates).strftime(DF), max(porfolio_dates).strftime(DF))))
                else:
                    months = 1
                    if i.survey_frequent == 'quarter': months = 4
                    if porfolio_dates:
                        if min(porfolio_dates) > i.start_date or max(porfolio_dates) < i.start_date + relativedelta(months= (months*i.number_of_frequent)):
                            raise ValidationError(_("Project timeline should be within Project's Porfolio timeline %s to %s"%(min(porfolio_dates).strftime(DF), max(porfolio_dates).strftime(DF))))
